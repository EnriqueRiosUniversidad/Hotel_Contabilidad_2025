...
const BalanceSYS = () => {
  ...
  const formatNumber = (n) => n.toLocaleString('es-PY');

  const exportToExcel = () => {
    const headers = ["codigo", "cuenta", "debe", "haber", "saldoDebe", "saldoHaber"];
    ...
    data.push(["Totales", "", totalDebe, totalHaber, totalSaldoDebe, totalSaldoHaber]);
    const worksheet = XLSX.utils.aoa_to_sheet([headers, ...data]);
    worksheet["!cols"] = [{ wch: 12 }, { wch: 30 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 12 }];
  ...
  const resultadoStyle = { color: resultadoEjercicio >= 0 ? 'green' : 'red', fontWeight: 'bold' };

  const chartData = {
    labels: registros.map(r => r.cuenta),
    datasets: [
      { label: 'Debe', backgroundColor: 'rgba(151, 209, 43, 0.6)', data: registros.map(r => r.debe || 0) },
      { label: 'Haber', backgroundColor: 'rgba(75, 112, 236, 0.6)', data: registros.map(r => r.haber || 0) },
    ]
  };
    ...
    <div className="d-flex">
      ...
      <div className="container py-3" style={{ marginLeft: "270px" }}>
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h2 className="text-center text-success fw-bold" style={{ fontFamily: "Georgia, serif" }}>Balance de Sumas y Saldos</h2>
          <div className="d-inline-flex gap-3 align-items-center">
            <div>
              <label htmlFor="periodo" className="form-label">Seleccionar Período</label>
              <select id="periodo" className="form-select form-select-sm" value={periodoId} onChange={(e) => { setPeriodoId(e.target.value); localStorage.setItem("periodoId", e.target.value); }}>
          ...
          <tbody>
            {registros.map((item, i) => (
              <tr key={i}>
                ...
                <td>{formatNumber(item.saldoHaber || 0)}</td>
              </tr>
            ))}
            <tr className="table-info">
              <td colSpan={2}><strong>Totales</strong></td>
              <td>{formatNumber(totalDebe)}</td>
              <td>{formatNumber(totalHaber)}</td>
              <td>{formatNumber(totalSaldoDebe)}</td>
              <td>{formatNumber(totalSaldoHaber)}</td>
            </tr>
            <tr className="table-success">
              <td colSpan={5} className="text-end"><strong>Resultado del ejercicio</strong></td>
              <td style={resultadoStyle}>{formatNumber(resultadoEjercicio)}</td>
            </tr>
          </tbody>
        </table>
      </div>
